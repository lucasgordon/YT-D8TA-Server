class DaysSincePublishedCalculator
  ...
  def calculate_all
    ...
    videos.each do |video|
      ...
      video.views.each do |view|
        ...
        next if days_since_published <= 0
  ...
  def process_day(days_since_published, day_data)
    ...
    sorted_videos.each_with_index do |(video_id, data), index|
      ...
      rank_change_since_day_1 = day_1_data ? day_1_data.rank - rank : nil
      day_over_day_rank_change = previous_day_data ? previous_day_data.rank - rank : nil
  ...
  def calculate_rank_slope(current_rank, day_1_rank, days_since_published)
    ...
    (day_1_rank - current_rank).to_f / (days_since_published - 1)
  end
